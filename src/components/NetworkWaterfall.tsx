...
const NetworkWaterfall: React.FC<Props> = ({ requests, width }) => {
  ...
  const hasActiveRequests = requests.some(
    req => req.state === 'pending' || 
           (req.endTime && now - req.endTime < INACTIVE_THRESHOLD)
  );
  ...
  const maxTime = hasActiveRequests ? now + FUTURE_BUFFER : 
                 requests.length > 0 ? Math.max(...requests.map(r => r.endTime || r.startTime)) + 1000 : 
                 now;
